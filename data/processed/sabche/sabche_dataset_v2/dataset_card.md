...
| split | `data/processed/sabche/sabche_split_v2_frozen.csv` (**test frozen**) |
...
python src/sabche/clean_sabche_spans.py        # B: sidecar + book verdicts
python src/sabche/prepare_sabche_split.py --val-frac 0.085 --test-frac 0.085 --version v2
python src/sabche/build_sabche_dataset.py --split-file data/processed/sabche/sabche_split_v2_frozen.csv \
    --out-dir data/processed/sabche/sabche_dataset_v2 --stats-json data/processed/sabche/sabche_dataset_v2_stats.json
```
...
- New groups are placed greedily, largest first, toward 83/8.5/8.5 by window
  count. The split_v3-fixed groups alone already fill 8.9% of test windows,
  so test received no new groups: every test book comes from split_v3's test
  set or the moved lamrim group.
- 240 groups: 34 with more than one book; the largest has 24 books
  (I048844DB).

| split | books | old / new | from split_v3 | windows | spans |
|---|---|---|---|---|---|
...
- Metric: `v2_metrics.py` lives in the team repo
  ...
  `scripts/tsawa/eval/eval_viterbi_iou.py` (`inclusive_iou`, `match_iou`, `score`).
  ...
  `scripts/tsawa/eval/span_metrics.py`: it does no one-to-one matching, scores
