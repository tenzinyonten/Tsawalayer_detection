...
| span source | `data/processed/sabche_spans_clean.csv` (dropped=False) |
| split | `data/processed/sabche_split_v2_frozen.csv` (**test frozen**) |
| metric (for training) | IoU ≥ 0.5, greedy one-to-one, inclusive offsets — same as tsawa/quotation |

## Pipeline

```bash
python scratch/audit_sabche.py          # A: audit (read-only)
python src/clean_sabche_spans.py        # B: sidecar + book verdicts
python src/prepare_sabche_split.py --val-frac 0.085 --test-frac 0.085 --version v2
python src/build_sabche_dataset.py --split-file data/processed/sabche_split_v2_frozen.csv \
    --out-dir data/processed/sabche_dataset_v2 --stats-json data/processed/sabche_dataset_v2_stats.json
```
...
On all 323 books at 12/12, before the 10-shared-spans rule, it was val 3.04% / test 6.02%. Linked
