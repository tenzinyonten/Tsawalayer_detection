...
- **Built:** 2026-09-15 by `src/build_tsawa_dataset.py`
- **Span source:** `data/processed/tsawa_spans_resolved.csv` (snapped boundaries + overlap resolve; `dropped=True` skipped)
- **Split file:** `data/processed/split_v2_frozen.csv` (greedy window-balanced, seed 123)
- **Repo selection:** `data/processed/tsawa_audit.csv`, `--source combined` (212 pechas: 123 new + 89 old)
...
- **Excluded spans:** 6 overlap stubs → `data/processed/dropped_spans_v2.csv`; 21,149 spans labeled
...
- Must-link reprint groups are kept within one split (see `data/processed/split_v2_frozen.md`).

## Relationship to v1

`data/processed/tsawa_dataset/` (v1) is kept for comparison. Same spans and
