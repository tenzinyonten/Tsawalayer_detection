...
- Labels from `/Users/tenzinyonten/layer_detection/data/processed/tsawa_spans_resolved.csv` (snapped boundaries + overlap resolve).
- Skip `dropped=True` (and empty stubs). **6** sidecar rows excluded; **21149** spans labeled.
- Cloned `Tsawa.yml` is not rewritten.
- Log of excluded sidecar rows: `data/processed/dropped_spans.csv`.
