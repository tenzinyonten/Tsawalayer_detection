...
Measured on the corpus: of 21,149 tsawa spans, 34.1% are immediately followed
by a closing particle (ཞེས་, ཅེས་, ཞེས་གསུངས, …) and only 0.6% contain one.
The particle marks where the root text ENDS and the commentary begins.

The dominant failure is over-inclusive prediction: 31.5% of gold spans are
swallowed by a prediction that runs past them. Where that overrun crosses a
particle, forbidding TSAWA on the particle's tokens forces the span to close
before it. No retraining — this changes only how the existing logits are
decoded.

Method: rebuild each window's text from its tokens, find every closer, and set
the B-TSAWA / I-TSAWA logits to -1e9 on the tokens covering it before Viterbi.

The honest expectation is a modest gain. It only fires where an overrun
actually crosses a particle, and IoU@0.5 already forgives small overruns.
