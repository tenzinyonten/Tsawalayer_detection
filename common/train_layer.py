#!/usr/bin/env python3
"""
train_layer.py (v5) — span detection for one layer (--label-name, default TSAWA),
with quotation available as an auxiliary class. Was common/train_layer.py; the layer name
is now a parameter so the same script trains tsawa and sabche.
...
def configure_label(name: str) -> None:
    """Fill SCHEMES / ENTITIES for the layer called `name` (e.g. TSAWA, SABCHE).
    The dicts are updated in place, so every function that reads them sees the
    chosen layer. Label ids are the same for every layer: O=0, B=1, I=2 (, E=3)."""
    global PRIMARY
