...
from ``tsawa/data/processed/tsawa_audit.csv`` (Phase 2). If the audit totals do
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    p.add_argument(
        "--out-dir",
        type=Path,
        default=Path("tsawa/data/processed/tsawa_dataset"),
        help="DatasetDict save_to_disk directory.",
    )
    p.add_argument(
        "--dropped-csv",
        type=Path,
        default=Path("tsawa/data/processed/dropped_spans.csv"),
...
def main(argv: list[str] | None = None) -> int:
    ...
    if use_sidecar:
        ...
        if sidecar_path.name == "tsawa_spans_merged.csv":
            extra = (
                ...
                "were merged (`tsawa/src/merge_tsawa_spans.py`).\n"
            )
        sidecar_note = (
    ...
    if multiclass:
        ...
        quote_note = (
            ...
            f"`Citation.yml` new batch, snapped by `tsawa/src/snap_quotation_boundaries.py`).\n"
