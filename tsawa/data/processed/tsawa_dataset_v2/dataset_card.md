...
- **Built:** 2026-09-15 by `common/build_tsawa_dataset.py`
- **Span source:** `tsawa/data/processed/tsawa_spans_resolved.csv` (snapped boundaries + overlap resolve; `dropped=True` skipped)
- **Split file:** `tsawa/data/processed/split_v2_frozen.csv` (greedy window-balanced, seed 123)
- **Repo selection:** `tsawa/data/processed/tsawa_audit.csv`, `--source combined` (212 pechas: 123 new + 89 old)
...
- **Excluded spans:** 6 overlap stubs → `tsawa/data/processed/dropped_spans_v2.csv`; 21,149 spans labeled
...
- Must-link reprint groups are kept within one split (see `tsawa/data/processed/split_v2_frozen.md`).

## Relationship to v1

`tsawa/data/processed/tsawa_dataset/` (v1) is kept for comparison. Same spans and
