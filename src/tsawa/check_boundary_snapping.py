...
Motivating case: P000207 span [92705:92820] starts with ``ྲོང`` — the tail
of ``གྲོང`` (grong), with the leading ``ག`` left just outside the span.
That is an off-by-one / mid-syllable start.

A syllable/word boundary is a tsheg (་), a shad (། / ༔ / ༎), whitespace
or newline, or the document start/end.

* start is clean if ``start == 0`` or ``text[start-1]`` is a boundary marker
* end is clean if ``text[end-1]`` (last char *inside* the span) is a marker

Diagnostic only — does not rewrite any spans. Helpers match
``spot_check_tsawa.py`` (audit ``n_spans`` / ``opf_root`` / nested ``.opf``).

Usage:
    python src/tsawa/check_boundary_snapping.py \\
        --audit-csv data/processed/tsawa/tsawa_audit.csv \\
        --raw-opf-dir data/raw_opf \\
        --out data/processed/tsawa/boundary_check.csv
"""
