...
------------------------------------
Same layout as Phase 1–2: a clone at ``data/raw_opf/<PECHA_ID>.opf/`` holds
the Pecha tree at that path or nested as ``<PECHA_ID>.opf/<PECHA_ID>.opf/``.
``base/v001.txt`` is one Unicode string; ``layers/v001/Tsawa.yml`` stores
**start-inclusive, end-exclusive** character spans into that string.

By default labels come from ``data/processed/tsawa/tsawa_spans_resolved.csv``
(snapped boundaries + overlap resolve). ``dropped=True`` rows are skipped.
Use ``--from-yaml`` to label raw YAML instead. Cloned ``Tsawa.yml`` is never
rewritten.

This script does **not** decide which repos have Tsawa. That list comes
from ``data/processed/tsawa/tsawa_audit.csv`` (Phase 2). If the audit totals do
not match the frozen Phase 2 counts, we abort instead of guessing.

BIO convention
--------------
Each subword token is labeled from its *character start offset* only:

* if ``span.start <= token_start < span.end`` → the token is inside that
  span (``B-TSAWA`` for the first such token of the span, else ``I-TSAWA``)
* otherwise → ``O``, even if the token's character range later overlaps a
  span (a token that *straddles* a boundary follows where it **starts**)

``isverse`` is ignored: old-batch Tsawa.yml files do not have it, so a
verse/prose distinction would be batch-confounded.

Windows are sliced from the **full-document** token/label arrays
(``max_length`` / ``stride``, token-index based). No training loop.
"""
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    p.add_argument(
        "--audit-csv",
        type=Path,
        default=Path("data/processed/tsawa/tsawa_audit.csv"),
    ...
    p.add_argument(
        "--out-dir",
        type=Path,
        default=Path("data/processed/tsawa/tsawa_dataset"),
        help="DatasetDict save_to_disk directory.",
    )
    p.add_argument(
        "--dropped-csv",
        type=Path,
        default=Path("data/processed/tsawa/dropped_spans.csv"),
        help="Log of spans excluded from labeling (zero-length YAML and/or resolver drops).",
    )
    p.add_argument(
        "--sidecar",
        type=Path,
        default=Path("data/processed/tsawa/tsawa_spans_resolved.csv"),
...
def write_dataset_card(
    ...
    *,
    ...
    lines = [
        ...
        "",
        "Built by `src/tsawa/build_tsawa_dataset.py` from Phase 2 `tsawa_audit.csv`.",
...
def main(argv: list[str] | None = None) -> int:
    ...
    if use_sidecar:
        ...
        if sidecar_path.name == "tsawa_spans_merged.csv":
            extra = (
                ...
                "were merged (`src/tsawa/merge_tsawa_spans.py`).\n"
            )
        sidecar_note = (
    ...
    if multiclass:
        ...
        quote_note = (
            ...
            f"`Citation.yml` new batch, snapped by `src/tsawa/snap_quotation_boundaries.py`).\n"
