...
from ``data/processed/tsawa_audit.csv`` (Phase 2). If the audit totals do
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    p.add_argument(
        "--audit-csv",
        type=Path,
        default=Path("data/processed/tsawa_audit.csv"),
    ...
    p.add_argument(
        "--out-dir",
        type=Path,
        default=Path("data/processed/tsawa_dataset"),
        help="DatasetDict save_to_disk directory.",
    )
    p.add_argument(
        "--dropped-csv",
        type=Path,
        default=Path("data/processed/dropped_spans.csv"),
        help="Log of spans excluded from labeling (zero-length YAML and/or resolver drops).",
    )
    p.add_argument(
        "--sidecar",
        type=Path,
        default=Path("data/processed/tsawa_spans_resolved.csv"),
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.limit is None and len(selected) != expected:
        raise SystemExit(
    ...
    if use_sidecar:
        ...
        if args.limit is None and args.source == "combined":
        ...
        sidecar_note = (
            f"- Labels from `{sidecar_path}` (snapped boundaries + overlap resolve).\n"
