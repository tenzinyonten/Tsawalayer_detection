...
Per book with a non-empty Chapter layer:

1. **Snap off-by-one edges.** A start/end cut inside a syllable is walked to the
   nearest boundary within ``MAX_WALK`` chars (start walks left; end walks
   right and stops *before* the boundary char, so the trailing shad stays
   outside the span, as in Sabche). Anything further is left unchanged.
2. **Trim overlaps** that snapping creates between neighbours.
...
OUT = ROOT / "chapter/data/processed/chapter_spans_clean.csv"
OUT_BOOKS = ROOT / "chapter/data/processed/chapter_book_verdicts.csv"
