...
Read-only, light (tokenizes the books, no model). Maps predicted token spans back
to characters, removes the duplicates that overlapping windows produce, and
matches them against the cleaned gold spans at IoU >= 0.5 in character space.
Prints per-book results, what kind of gold spans are missed, and what the false
positives look like, and writes missed/false-positive lists as CSV.

Use it on the VALIDATION dump. Reading test errors and then changing the model
would use up the frozen test split.

Usage:
    python chapter/src/analyze_chapter_errors.py --spans scratch/chapter/chapter_val_spans.jsonl \\
...
def main(argv=None) -> int:
    ...
    ap.add_argument("--out-dir", type=Path, default=ROOT / "scratch/chapter")
