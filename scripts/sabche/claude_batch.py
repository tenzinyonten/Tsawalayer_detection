...
and per-window cache format of scripts/tsawa/gemini/run_tsawa_zeroshot.py, so the
...
requests and reports what it would send.
    ...
    python scripts/sabche/claude_batch.py plan --groups 3

    # 2. dry run of one group (still offline), then submit it
    python scripts/sabche/claude_batch.py create --group g1 --books P000083 ...
    python scripts/sabche/claude_batch.py create --group g1 --books P000083 ... --yes

    # 3. wait, then download one file per window
    python scripts/sabche/claude_batch.py poll  --group g1
    python scripts/sabche/claude_batch.py fetch --group g1

    # 4. score (no API key needed)
    python scripts/tsawa/gemini/run_tsawa_zeroshot.py --layer sabche --books ... \\
...
RUNNER = ROOT / "scripts/tsawa/gemini/run_tsawa_zeroshot.py"
