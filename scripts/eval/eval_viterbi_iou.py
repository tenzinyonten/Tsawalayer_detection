...
Two things this adds over plain argmax evaluation:

1. VITERBI DECODING. Argmax picks each token independently, so it can emit
   I-TSAWA straight after O — a span with no start. Viterbi finds the best
   LEGAL label sequence instead, using a transition matrix where illegal
   moves cost -1e9. A ``break_penalty`` is charged every time a span exits,
   which discourages chopping one span into several.

2. IoU@0.5 SPAN F1, matching src/layer_detection/v2_metrics.py in the team
   repo: inclusive offsets (a span [3,7] is 5 tokens, not 4), greedy
   best-first one-to-one matching, IoU = intersection / union.

Both matter for comparability. The team's yigchung single-layer model reports
0.389 iou50_f1; a number computed any other way is not comparable to it.
...
def main():
    ...
    ap.add_argument("--dataset", default="data/processed/tsawa_dataset_v2")
    ...
    ap.add_argument("--out", default="scratch/viterbi_eval.json")
